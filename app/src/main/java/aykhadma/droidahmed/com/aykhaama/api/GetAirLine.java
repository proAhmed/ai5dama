...
public class GetAirLine extends AsyncTask<String, Void,  AirLineMain > {

	private   String URL =  "http://ai5adma.com/API/ar/airline";
	private ProgressDialog dialog;
	private OnLoadingComplete callback;
	private Context context;

	public GetAirLine(Context context, OnLoadingComplete cb) {
	...
	@Override
	protected  AirLineMain  doInBackground(String... params) {
		...
		if (responseJSON != null && responseJSON.length() > 1) {
			...
			gson = gb.create();
			try {
				obj = gson.fromJson(responseJSON, AirLineMain.class);
	...
	@Override
	protected void onPostExecute( AirLineMain  result) {
		if (dialog.isShowing()) {
			dialog.dismiss();
		}
		if (result != null) {
