...
public class GetVideos extends AsyncTask<String, Void, VideoModelMain> {

	private   String URL =  "http://ai5adma.com/API/ar/video";
	private ProgressDialog dialog;
	private OnLoadingComplete callback;
	private Context context;

	public GetVideos(Context context, OnLoadingComplete cb) {
	...
	@Override
	protected VideoModelMain doInBackground(String... params) {
		...
		if (responseJSON != null && responseJSON.length() > 1) {
			...
			gson = gb.create();
			try {
				obj = gson.fromJson(responseJSON, VideoModelMain.class);
	...
	@Override
	protected void onPostExecute( VideoModelMain result) {
		if (dialog.isShowing()) {
			dialog.dismiss();
		}
		if (result != null) {
