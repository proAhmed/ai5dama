...
public class CheckOutWholeSale extends AsyncTask<ArrayList<CartQuantity>, Void, ResponseOfCheckOut> {
	...
	public CheckOutWholeSale(Context context, OnLoadingComplete cb) {
		dialog = new ProgressDialog(context);
		callback = cb;
		this.context = context;
		url = "http://ai5adma.com/API/ar/wscheckout/buy?token="+new StoreData(context).getToken();
	Log.d("mmmuuttt",url);
	}
	...
	protected ResponseOfCheckOut doInBackground(ArrayList<CartQuantity>... params) {
		...
		try {
			...
			Log.d("mmuuttt",responseJSON);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			Log.d("mmmuu",e.toString());
		}
...
for(int i=0;i<cartQuantities.size();i++){
	...
	itemJson.put("ID",cartQuantities.get(i).getProductID());
