...
public class GetSlide extends AsyncTask<String, Void, SlideShowResponse> {

	private   String URL =  "http://ai5adma.com/API/ar/home/slideshow";
	private ProgressDialog dialog;
	private OnLoadingComplete callback;
	private Context context;

	public GetSlide(Context context, OnLoadingComplete cb) {
	...
	@Override
	protected SlideShowResponse doInBackground(String... params) {
		...
		if (responseJSON != null && responseJSON.length() > 1) {
			...
			gson = gb.create();
			try {
				obj = gson.fromJson(responseJSON, SlideShowResponse.class);
	...
	@Override
	protected void onPostExecute(SlideShowResponse result) {
		if (dialog.isShowing()) {
			dialog.dismiss();
		}
		if (result != null) {
